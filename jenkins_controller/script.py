...
class ScriptGenerator():
    ...
    CMD_MUGEN_DEP_INSTALL = {"debian": "{0} apt-get install -y sudo file expect git psmisc iputils-ping make python3-paramiko python3-six tar jq",
                             "fedora": "{0} dnf install -y sudo file expect git psmisc make iputils python3-six python3-paramiko tar jq",
                             "archlinux": "{0} pacman --need --noconfirm -S sudo file expect git psmisc make iputils python-six python-paramiko tar jq",
                             "gentoo": "{0} emerge --color=n --getbinpkg --noreplace --autounmask=y app-admin/sudo sys-apps/file dev-tcltk/expect dev-vcs/git sys-process/psmisc dev-build/make dev-python/paramiko dev-python/six app-arch/tar app-misc/jq"}


    def __init__(self, gen_type: str, disto_type: str, cfg: dict):
    ...
    def _gen_mugen_test_cmds(self):
        ...
        self.cmds.append(CmdGenerator.CMD_MUGEN_DEP_INSTALL[self.disto_type])
    ...
    def _gen_upgrade_cmds(self):
        self.cmds.append(CmdGenerator.CMD_UPGRADE[self.disto_type])

    def get_script(self) -> str:
        cmds = ""
        for c in self.cmds:
            cmds += c + "\n"
        return cmds.format("sudo" if self.sudo else "", self.disto_type)
